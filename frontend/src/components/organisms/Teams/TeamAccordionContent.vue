...
onMounted(async() => {
	const dataStudents = await getStudentsByTeamId(props.teamId)
	students.value = dataStudents
	const dataCriteria = await getCriteria(props.teamId)
	criteria.value = dataCriteria
})
